...
class ProjectIssueAccess(models.Model):
    ...
    def unlink(self):
        for record in self:
            ...
            if access_control_rules_id and access_control_rules_id.issues_delete == False:
                raise exceptions.Warning(('Error'), ('You cannot delete an issue that you did not have access.'))
    ...
    def write(self, vals):
        for record in self:
            access_control_rules_id = self.env['project.access.control.rules'].search([('user_id.id','=', self.env.user.id), ('project_id.id','=',record.project_id.id)])
            if access_control_rules_id and access_control_rules_id.issues_write == False:
                raise exceptions.Warning(('Error'), ('You cannot edit an issue that you did not have access.'))
    ...
    def create(self, vals):
        ...
        if access_control_rules_id and access_control_rules_id.issues_create == False:
            _logger.info('000000000000000000000000000000 %s', access_control_rules_id)
            raise exceptions.Warning(('Error'), ('You cannot create an issue that you did not have access.'))
